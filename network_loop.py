...
async def run_network_loop(supervisor: Supervisor):
    ...
    # Register handlers
    message_loop.register_handler(
        "feedback_request", lambda content: supervisor.get_feedback(content)
    )
    message_loop.register_handler(
        "command_request", lambda content: supervisor.run_command(content)
    )
