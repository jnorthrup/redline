# 1663711c-ea89-4751-81a3-f4f3bb047286
class Supervisor:
        ...
        try:
            ...
            if self.provider_manager.current_provider:
                ...
                feedback = self.provider_manager.current_provider.generate(
                    prompt, self.message_handler.messages[0]["content"]
                )
                if feedback:
    ...
    def _process_feedback(self, feedback: str, change: str = None) -> str:
        ...
        try:
            ...
            recent_feedback = self.memory_manager.get("feedback_history")[
                -5:
            ]  # Last 5 feedback entries
            if (
                len(recent_feedback) > 1
            ):  # Only analyze patterns if we have multiple entries
    ...
    def _analyze_change_patterns(
        ...
        try:
            ...
            frequently_modified = [
                f for f, count in modified_files.items() if count > 1
            ]
            if frequently_modified:
                patterns.append(
                    f"Frequently modified files: {', '.join(frequently_modified)}"
                )
            ...
            for i, change in enumerate(code_changes[:-1]):
                for next_change in code_changes[i + 1 :]:
                    if (
                        change.get("change_context")
                        and next_change.get("change_context")
                        and any(
                            word in next_change["change_context"]
...
if __name__ == "__main__":
    ...
    try:
        logger.debug(
            f"Initializing provider with config: api_base={config.api_base}, model={config.model_name}"
        )

        # Initialize provider
        provider = QwenProvider(
            config={"api_base": config.api_base, "model": config.model_name}
        )
        ...
        while True:
            ...
            if cmd == "quit":
                ...
                if recent_feedback:
                    pattern_analysis = supervisor._analyze_change_patterns(
                        recent_feedback
                    )
                    if pattern_analysis:
                        print("\nDetected Patterns:")
                ...
                try:
                    with open(file_path, "r") as f:
                        content = f.read()
                    feedback = supervisor.get_feedback(
                        f"File: {file_path}\n\n{content}"
                    )
        ...
        logger.error(
            f"Failed to connect to LM Studio API at {config.api_base}. Is LM Studio running?"
        )
