...
def get_help_output(command: str) -> Optional[str]:
    ...
    try:
        ...
        output = subprocess.check_output([command, '--help'], stderr=subprocess.STDOUT, text=True, timeout=5)
        return output
...
def detect_llm_tools() -> List[Tuple[str, Dict[str, Any]]]:
    ...
    if get_help_output('ollama'):
        ...
        try:
            ...
            logging.debug(f"Request to Ollama API: GET {api_base}/api/tags")  # Added debug log
            if response.status_code == 200:
                ...
                if models:
                    ...
                    logging.debug(f"Ollama models retrieved: {models}")  # Added debug log
        except Exception as e:
    ...
    if lm_studio_base and lm_studio_base.endswith('v1'):
        ...
        try:
            ...
            logging.debug(f"Request to LM Studio API: GET {lm_studio_base}/models")  # Added debug log
            if response.status_code == 200:
                tools.append(('lm_studio', {
                    'api_base': lm_studio_base,
                    'api_key': os.getenv('OPENAI_API_KEY')
                }))
                logging.debug("LM Studio models retrieved successfully")  # Added debug log
        except Exception as e:
    ...
    while True:
        try:
            ...
            if "```" in ai_response and not is_stdin_pipe():
                choice = get_user_input("\nWould you like me to execute any of these commands? (y/n): ").lower()
                if choice == 'y':
